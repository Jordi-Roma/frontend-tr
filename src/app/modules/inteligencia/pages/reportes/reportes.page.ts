...
export class ReportesPage implements OnInit, OnDestroy {
  ...
  protected readonly filtros = signal<ReporteFiltros>({
    tipo: 'VENTAS', fecha_desde: this.fechaMes(), fecha_hasta: this.fechaHoy(), sucursal_id: null, agrupacion: 'DIA', solo_bajo_stock: false,
  });
  ...
  protected actualizarFiltro(campo: keyof ReporteFiltros, event: Event): void {
    ...
    const siguiente = campo === 'sucursal_id' ? (value ? Number(value) : null) : value;
    this.filtros.update((actual) => ({ ...actual, [campo]: siguiente } as ReporteFiltros));
  }
